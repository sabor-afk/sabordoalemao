...
function abrirProduto(index) {
    ...
    if (temNutri) {
        ...
        let colunas = [];
        for (const nutriente of Object.values(nutri)) {
            if (typeof nutriente === 'object' && nutriente !== null) {
                colunas = Object.keys(nutriente).filter(k => k !== 'vd');
                if (colunas.length > 0) break;
            }
        }
        
        // Se não encontrou colunas, usa padrão
        if (colunas.length === 0) colunas = ['100g', '40g'];
        
        // Função auxiliar para renderizar uma linha de nutriente
        const renderLinhaNutriente = (nome, dados) => {
            if (!dados || !dados[colunas[0]]) return '';
            
            let html = `<tr style="border-bottom: 1px solid #ddd;">
        ...
        // Montar tabela
        let linhasTabela = '';
        linhasTabela += renderLinhaNutriente('Valor energético (kcal)', nutri.valor_energetico);
        linhasTabela += renderLinhaNutriente('Carboidratos (g)', nutri.carboidratos_totais);
        linhasTabela += renderLinhaNutriente('Açúcares totais (g)', nutri.acucares_totais);
        linhasTabela += renderLinhaNutriente('Açúcares adicionais (g)', nutri.acucares_adicionais);
        linhasTabela += renderLinhaNutriente('Proteínas (g)', nutri.proteinas);
        linhasTabela += renderLinhaNutriente('Gorduras totais (g)', nutri.gorduras_totais);
        linhasTabela += renderLinhaNutriente('Gorduras saturadas (g)', nutri.gorduras_saturadas);
        linhasTabela += renderLinhaNutriente('Gorduras trans (g)', nutri.gorduras_trans);
        linhasTabela += renderLinhaNutriente('Fibras alimentares (g)', nutri.fibras_alimentares);
        linhasTabela += renderLinhaNutriente('Sódio (mg)', nutri.sodio);
        
        // Header dinâmico
        let headerColunas = '';
        for (const col of colunas) {
            headerColunas += `<td style="padding: 2px 4px; border-right: 1px solid #333; font-weight: bold; text-align: center; font-size: 0.65rem;">${col}</td>`;
        }
        
        const nutriHTML = `
            <div style="margin-top: 5px; overflow-x: auto; margin-left: -20px; margin-right: -300px; padding-left: 20px; padding-right: 20px;">
                <table style="width: 100%; border-collapse: collapse; font-size: 0.7rem; border: 1px solid #333; min-width: 100%;">
                    <thead>
                        <tr style="background: #333; border-bottom: 1px solid #333;">
                            <td style="padding: 2px 4px; border-right: 1px solid #333; font-weight: bold; width: 30%; font-size: 0.65rem;"></td>
                            ${headerColunas}
                            <td style="padding: 2px 4px; font-weight: bold; text-align: center; font-size: 0.65rem;">%VD</td>
                        </tr>
                    </thead>
                    <tbody>
                        ${linhasTabela}
                ...
                <p style="font-size: 0.6rem; color: #999; margin-top: 2px; margin-bottom: 0;">* % VD pela porção</p>
            </div>
        `;
        document.getElementById('prodNutri').innerHTML = nutriHTML;
